...
(function () {
    ...
    function activate() {
        if (window.MediaCompat && typeof window.MediaCompat.markRemoteMode === 'function') {
            window.MediaCompat.markRemoteMode();
        }
    ...
    function onKeyDown(event) {
        ...
        if (isSelect) {
            if (isPlayerFullscreen() && playerAction('toggle')) {
                event.preventDefault();
                event.stopImmediatePropagation();
                return;
            }
            if (input) return;
            // Fullscreen mode treats OK as play/pause even when a TV browser
            // resets focus to body or leaves it on a transient player control.
            const handled = target && target.matches('[data-tv-player-surface]')
                ? playerAction('toggle')
                : false;
        ...
        // A direction with no LRUD destination must not fall through to the
        // legacy player shortcuts and unexpectedly seek or change volume.
        event.preventDefault();
        event.stopImmediatePropagation();
    }
